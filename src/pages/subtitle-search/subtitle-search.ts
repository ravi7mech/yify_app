...
import { Movies,TMDbMovieResults,TMDbResponse} from '../../models/moviesjson';
...
@Component({
  ...
  templateUrl: 'subtitle-search.html',
...
getNativeLanguage(movie:TMDbMovieResults){
  ...
  let english_name='';
  for (let i = 0; i < this.langjson.length; i++) {
...
retryVoiceMessage(msg:string) {
  ...
  getItems(val:any) {
   this.showText = false;
    if (val && val.trim() != '') {
      ...
      this.textinput = val;
      this.moviepage = 1;
      this.yifyprovider.searchTMDbQueryData(val,this.moviepage).subscribe(res=>{
        this.asyncCall = false;   
        if(res.results.length >0)   
        this.submovielist = res.results;
        else
        this.showText = true;
  ...
  doInfinite(infiniteScroll) {
    ...
    if (!this.asyncCall) {
      ...
      this.yifyprovider.searchTMDbQueryData(this.textinput,this.moviepage).subscribe(res=>{
        infiniteScroll.complete();
        if(res.results.length > 0)
        res.results.forEach(element => {
          this.submovielist.push(element);
        });
        else
        this.presentToast("No more results found!")
        console.log(this.submovielist)  
       })
    } else {
      infiniteScroll.complete();
